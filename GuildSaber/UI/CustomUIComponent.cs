...
public abstract class CustomUIComponent : MonoBehaviour
{
    ...
    protected virtual string GetViewDescription() { return string.Empty; }
    ...
    public BSMLParserParams Parse(Transform p_Parent)
    {
        try {
            ...
            if (l_Resource == string.Empty) l_Resource = GetViewDescription();
            ...
            var l_NewEx = new Exception($"Error during parsing bsml of component {name} : maybe path is invalid ?");
            throw l_NewEx;
        }
    }
}