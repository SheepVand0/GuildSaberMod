...
internal class ModFlowCoordinator : CustomFlowCoordinator
{
    ...
    {
        if (_modViewController == null) _modViewController = BeatSaberUI.CreateViewController<PlaylistViewController>();

        if (_LeftModViewController == null) _LeftModViewController = BeatSaberUI.CreateViewController<LeftModViewController>();

        return (_modViewController, _LeftModViewController, null);
    }

    protected override void OnShow() { _modViewController.Init(GSConfig.Instance.SelectedGuild); }
}