...
internal class GuildSelectionMenuCell
{
    ...
    [UIAction("#post-parse")] private void _PostParse()
    {
        ...
        {
            if (GuildSelectionMenu.m_CategorySelectionFlowCoordinator == null) GuildSelectionMenu.m_CategorySelectionFlowCoordinator = BeatSaberUI.CreateFlowCoordinator<CategorySelectionFlowCoordinator>();
...
internal class GuildSelectionMenu : ViewController<GuildSelectionMenu>
{
    ...
    {
        m_Guilds.Clear();
        foreach (GuildData l_Guild in GuildSaberModule.AvailableGuilds) m_Guilds.Add(new GuildSelectionMenuCell(l_Guild.Name, l_Guild.Description, l_Guild.Logo));
