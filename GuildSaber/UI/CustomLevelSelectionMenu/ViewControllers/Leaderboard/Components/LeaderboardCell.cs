...
namespace GuildSaber.UI.CustomLevelSelectionMenu.ViewControllers.Leaderboard.Components
{
    internal class LeaderboardCell : XUISecondaryButton
    {
        ...
        private void OnCreation(CSecondaryButton p_Button)
        {
            ...
            XUIHLayout.Make(
                XUIHLayout.Make(
                    XUIText.Make("1 - ").Bind(ref m_RankText),
                    XUIText.Make("Sheep").Bind(ref m_NameText),
                    XUIText.Make("0.00").Bind(ref m_PointsText)
                ).ForEachDirect<XUIText>((x) =>
                 {
                     x.SetAlign(TMPro.TextAlignmentOptions.Left);
                     x.SetFontSize(l_FontSize);
                 })
                  .SetWidth(40),
                XUIHLayout.Make(
                        XUIText.Make("69000000").Bind(ref m_ScoreText).SetColor(new Color(1, 1, 1, 0.7f)),
                        XUIText.Make("69.00%").Bind(ref m_AccText)
                ).ForEachDirect<XUIText>((x) =>
                {
                    x.SetAlign(TMPro.TextAlignmentOptions.Right);
                    x.SetFontSize(l_FontSize);
                })
                .SetWidth(40)
            )
