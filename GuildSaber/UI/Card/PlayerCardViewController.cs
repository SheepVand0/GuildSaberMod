...
internal class PlayerCardViewController : ViewController<PlayerCardViewController>
{
    ...
    public void UpdateCardColor()
    {
        ...
        foreach (PlayerRankUI? l_Current in m_Ranks) l_Current.SetColor(GSConfig.Instance.UseCustomPointsColor && m_AllowCustomCardGradient ? GSConfig.Instance.CustomPointsColor : PlayerCardUI.m_Player.Color?.ToUnityColor() ?? Color.white);
    }
    ...
    private void OnPPClick()
    {
        ...
        if (GSConfig.Instance.ShowSettingsModal) ShowSettings();
    }
    ...
    public void Refresh()
    {
        try {
            ...
            if (l_SetCardActive) { PlayerCardUI.SetCardActive(true); }
            else {
        ...
        catch (Exception l_E) { GSLogger.Instance.Error(l_E, nameof(PlayerCardViewController), nameof(Refresh)); }
    }
    ...
    public void UpdateTime(OptimizedDateTime p_Time) { m_PlayTimeText.text = GSConfig.Instance.ShowPlayTime ? string.Join(":", p_Time.Hours.ToString("00"), p_Time.Minutes.ToString("00"), p_Time.Seconds.ToString("00")) : " "; }
    ...
    ///     Show
    /// </summary>
    public void ShowSettings() { m_ModalView.Show(true); }
    ...
    private void OnButtonRefreshCardClicked() { PlayerCardUI.RefreshCard(true); }
    ...
    [UIAction("ResetPosMenu")] private void ResetPosMenu() { PlayerCardUI.ResetMenuCardPosition(); }

    [UIAction("ResetPosGame")] private void ResetPosInGame() { PlayerCardUI.ResetInGameCardPosition(); }
}