...
public class OptimizedDateTime
{
    ...
    public bool AddSecondAndUpdateClock(float p_Value)
    {
        ...
        if (!(Minutes >= 60)) return l_ShouldUpdate;

        Hours += Minutes / 60;
        Minutes = 0;

        if (!(Hours >= 24)) return l_ShouldUpdate;

        Day += Hours / 24;
        Hours = 0;

        if (!(Day >= GetCurrentMonthDayCount())) return l_ShouldUpdate;

        Month += Day / GetCurrentMonthDayCount();
        Day = 0;

        if (!(Month >= 12)) return l_ShouldUpdate;
    ...
    /// </summary>
    /// <returns></returns>
    private int GetCurrentMonthDayCount() { return m_DaysInMonth[Month]; }
}